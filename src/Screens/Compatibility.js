...
import { OneToOne, Names } from '../ReduxFloder/action';
...
const Compatibility = ({ navigation }) => {
    ...
    const nav = () => {
        ...
        setloading(true)
        let bodyFormData = { 'userid': Data.userdetail.userid, 'type': 2, 'name': name, 'dob': day, 'gender': selectedLanguage, 'email': mail }
        console.log(bodyFormData);
        axios({
            method: 'POST',
            url: `https://astar.7kstartup.com/public/api/newothercompatibilitycheck`, // OneToOne Compatibilty check API
            data: bodyFormData,
    ...
    const EmaiShare = () => {
        ...
        if (mail != "") {
            ...
            axios({
                method: 'POST',
                url: `https://astar.7kstartup.com/public/api/newothercompatibilitycheck`, // Signup API
                data: bodyFormData,
                config: { headers: { 'Content-Type': 'multipart/form-data' } }
            })
                .then(({ data, status }) => {
                    ...
                    dispatch(OneToOne(data))
                    setloading(false)
                    let FormData = { 'otherpersonid': data.otheruser_detail.otherpersonid, 'userid': Data.userdetail.userid, 'module_type': 2, 'email': mail }
                                    ...
                                    <View style={{ backgroundColor: '#fff', borderRadius: 12, padding: 10, margin: 10, zIndex: 111111 }}>
                                        ...
                                        <View style={{ marginLeft: 25, marginTop: 10, marginBottom: 5, borderBottomWidth: 0.2, marginRight: 25, }}>
                                            ...
                                            <TextInput style={{ borderBottomWidth: 1, color: '#DA0AE4', fontSize: 16, borderBottomColor: '#EBEBEB', marginLeft: -5, fontFamily: 'AvertaStd-Regular' }}
                                                placeholder='Enter Your Email' placeholderTextColor='#DA0AE4'
